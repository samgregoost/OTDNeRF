...
class Camera:
  ...
  def perturb_camera(self, R, T, focal_points=-100.0, noise_scale=0.2):
  ...
  def get_perturbed_camera(self, noise):
      R_new, T_new = self.perturb_camera(self.orientation, self.position, noise_scale=noise)

     # R_new = self.orientation + noise * np.random.uniform(size=self.orientation.shape)

    #  T_new = self.position + noise * np.random.uniform(size=self.position.shape)

      new_camera = Camera(
          orientation=R_new.copy(),
          position=T_new.copy(),
          focal_length=self.focal_length,
          ...
          image_size=np.array((int(round(self.image_size[0] )),
                               int(round(self.image_size[1] )))),
      )
