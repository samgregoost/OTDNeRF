...
class Load_iphone_data():
    def __init__(self,
        ...
        self.add_cam = False
      #  if data_length != 1:
          #  self.all_img = self.all_img[:data_length]
      #    self.all_img = self.all_img[:int(len(self.all_img) * 3/4)]
        ...
        self.all_time = [meta_json[i]['appearance_id'] for i in self.all_img]
        max_time = max(self.all_time)
        self.all_time = [meta_json[i]['appearance_id'] / max_time for i in self.all_img]
        ...
        self.all_cmask = [f'{datadir}/covisible/{int(1 / ratio)}x/val/{i}.png' for i in self.temp_imgs]
